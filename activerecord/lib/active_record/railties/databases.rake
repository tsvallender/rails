  ...
  namespace :schema do
    ...
    task dump: :load_config do
      ActiveRecord::Tasks::DatabaseTasks.with_temporary_pool_for_each do |pool|
        ...
        ActiveRecord::Tasks::DatabaseTasks.dump_schema(db_config, db_config.schema_format(check_env: true))
      end
    ...
    namespace :load do
      ActiveRecord::Tasks::DatabaseTasks.for_each(databases) do |name|
        ...
        task name => "db:test:purge:#{name}" do
          ActiveRecord::Tasks::DatabaseTasks.with_temporary_pool_for_each(name: name) do |pool|
            db_config = pool.db_config
            ActiveRecord::Tasks::DatabaseTasks.load_schema(db_config, db_config.schema_format(check_env: true))
          end
        end
      end
    end
