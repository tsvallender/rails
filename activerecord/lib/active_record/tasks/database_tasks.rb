...
module ActiveRecord
  module Tasks # :nodoc:
    ...
    module DatabaseTasks
      ...
      def schema_up_to_date?(configuration, file = nil)
        db_config = resolve_configuration(configuration)
      ...
      def reconstruct_from_schema(db_config, file = nil) # :nodoc:
        file ||= schema_dump_path(db_config, db_config.format)
        ...
        with_temporary_pool(db_config, clobber: true) do
          if schema_up_to_date?(db_config, file)
            ...
            purge(db_config)
            load_schema(db_config, db_config.format, file)
          end
        rescue ActiveRecord::NoDatabaseError
          create(db_config)
          load_schema(db_config, db_config.format, file)
      ...
      def dump_schema(db_config, format) # :nodoc:
      ...
      def load_schema_current(file = nil, environment = env)
        each_current_configuration(environment) do |db_config|
          with_temporary_connection(db_config) do
            load_schema(db_config, db_config.schema_format(check_env: true), file)
          end
        end
      end
